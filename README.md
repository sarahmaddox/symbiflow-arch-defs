 ...
 * `<arch name>` - Architecture definitions containing everything unique to a
   particular architecture for
 ...
 * `<arch name>/devices/` - Top level description for an actual device.
   ...
   * `<arch name>/devices/<device name>-virt` - Verilog to Routing architecture
     definitions generally are not able to able to generate the **exact** model
     of many FPGA routing interconnects, but this is a pretty close. Frequently
     for testing we want something close to an actual device that isn't the real
     thing.

 * `<arch name>/primitives/` - The primitives that make up the architecture.
   These are generally used inside the tiles. What exactly a "primitive" is,
   is a bit fuzzy. Use your best judgment.
...
The `make env` command uses a [conda](https://conda.io) containing a precompiled
Python environment with required tools, Yosys and Verilog to Routing.
